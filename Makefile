...
docker-artisan: ## Roda o artisan
	docker exec $(CONTAINER_BACK) /bin/bash -c "php artisan $(wordlist 2,$(words $(MAKECMDGOALS)),$(MAKECMDGOALS))"

docker-prod: ## Executa o build do frontend para producao
	docker exec $(CONTAINER_FRONT) /bin/bash -c "npm run build"
